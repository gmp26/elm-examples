...
pieces : [Piece]
pieces = 
  zip (map (shift (4, 0)) [line, Tet.square, zpiece, spiece, jpiece, lpiece, tpiece])
...
pieceToForm fblockSize (tr, color) =
  ...
  let blocks = map (flip (B.toForm fblockSize) color) translate in
  group blocks
...
piece = Control.rotate CW <| shift (0,1) zpiece
