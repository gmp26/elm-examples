...
absLoc (Location player loc) = 
    if  | loc < 0          -> loc
        | loc >= bOARDSIZE -> loc
        | otherwise        -> (sTARTOFFSET * player + loc) % bOARDSIZE
...
newGame gs =
    { gs | tokenLoc <- initTokenLoc 
         , currentPlayer <- gs.randSeed % pLAYERCNT
    }

getLocation : GameState -> Token -> Location
getLocation gs (Token p t) = 
    case Dict.get (p,t) gs.tokenLoc of
    Just loc -> loc
...
throwDice gs = 
    ...
    in { gs | die <- (z % dIESIZE) + 1
            , randSeed <- z // dIESIZE
       }

seedRand :  Int -> GameState -> GameState
seedRand seed gs = { gs | die <- (seed % dIESIZE) + 1 }

nextPlayer gs = 
    { gs | currentPlayer <- (gs.currentPlayer + 1) % pLAYERCNT }
